...
module.exports = function(path) {
  return new RSVP.Promise(function(resolve, reject) {
    ...
    // TODO: parse `Title` as meta (or use first #h1 if not present)
    // TODO: case-insensitive match for allowed meta keys
    // TODO: put first h1 in content
    ...
    fs.createReadStream(path)
      ...
      .on('data', function(line) {
        ...
        if (isMeta) {
          ...
          // If we hit a line with `=` underneath, our previous line is a h1
          if (/^=/.test(line)) {
            isMeta = false;
